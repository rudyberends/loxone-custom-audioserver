...
import { getZoneById, sendCommandToZone } from '../../backend/zonemanager';
...
const AUDIO_COMMANDS_PATTERN = 'audio/\\d+/(on|off|play|resume|pause|queueminus|queueplus|volume|test)';
...
async function audioGetQueue(url: string) {
...
async function audioCfgGetMediaFolder(url: string) {
...
async function audioCfgGetPlaylists(url: string) {
...
function handleSecureHello(trimmedUrl: string) {
  ...
  const pub_key = parts[3]; // Assuming public key is the fourth part
  ...
  switch (true) {
    ...
    case trimmedUrl.startsWith(COMMAND_AUDIO_CFG_SETCONFIG):
      try {
        ...
        config.audioserver!.musicCFG = JSON.parse(str);
        return emptyCommand(trimmedUrl, { crc32: config.audioserver!.musicCRC, extensions: [] });
      } catch (error) {
        logger.error(`[${name}] Error processing setconfig: ${error}`);
        return unknownCommand(trimmedUrl);
      }
